...
#TODO: use iptables to forward from eth0 to wlan0 (now in enable_NAT.sh)
...
echo 'ExecStart=/bin/bash -c "nmcli radio wifi off && rfkill unblock wlan && ifconfig wlan0 192.168.42.1/24 up && sleep 1 && service isc-dhcp-server restart && service hostapd restart && /home/zoef/zoef_sd_card_image/enable_NAT.sh"' >> /lib/systemd/system/zoef_ap.service
echo '' >> /lib/systemd/system/zoef_ap.service
